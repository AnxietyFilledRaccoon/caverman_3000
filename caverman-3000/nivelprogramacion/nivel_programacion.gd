...
func _ready():
	$personaje/AnimatedSprite2D.animation_finished.connect(_on_animation_finished)
...
func _process(delta):
	if Input.is_action_just_pressed("hit") and not codeado:
		register_hit()

func register_hit():
	current_hits += 1
	print("Golpes:", current_hits)
	$personaje/AnimatedSprite2D.play("hit")
